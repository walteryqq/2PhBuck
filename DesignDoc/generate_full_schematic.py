...
def draw_schematic():
    ...
    # ---------------------------------------------------------
    # 1. Main Power Stage Rails (y = 3.0 is top rail, y = 1.6 is bottom rail)
    # ---------------------------------------------------------
    y_top = 3.2
    y_bot = 1.8
    
    # Bottom rail (Ground)
    plt.plot([0.5, 9.2], [y_bot, y_bot], color='#475569', lw=1.5)
    # Top rail sections
    plt.plot([0.5, 2.2], [y_top, y_top], color='#475569', lw=1.5)
    plt.plot([2.8, 3.4], [y_top, y_top], color='#475569', lw=1.5)
    plt.plot([3.8, 4.4], [y_top, y_top], color='#475569', lw=1.5)
    plt.plot([5.5, 9.2], [y_top, y_top], color='#475569', lw=1.5)
    ...
    ax.text(src_x - 0.4, (y_top+y_bot)/2, 'v̂_in(s)', fontsize=9.5, fontweight='bold', color='#1E293B', ha='right', va='center')
    ...
    lx_start = 4.4
    lx_end = 5.2
    # Draw a clean inductor coil symbol using overlapping arcs
    ...
    ax.text(c1_x + 0.25, y_top - 0.25, 'C1 = 5000 μF', fontsize=8, color='#475569')
    ax.text(c1_x + 0.25, y_bot + 0.45, 'Resr1 = 1.0 mΩ', fontsize=8, color='#475569')
    ...
    ax.text(c2_x + 0.25, y_top - 0.25, 'C2 = 100 μF', fontsize=8, color='#475569')
    ax.text(c2_x + 0.25, y_bot + 0.45, 'Resr2 = 0.3 mΩ', fontsize=8, color='#475569')
    ...
    ax.text(rl_x + 0.25, (y_top+y_bot)/2, 'R_load = 0.124 Ω', fontsize=8.5, color='#1E293B')
    ...
    # Output Voltage node
    plt.plot([9.2, 9.8], [y_top, y_top], color='#475569', lw=1.5)
    plt.plot([9.2, 9.8], [y_bot, y_bot], color='#475569', lw=1.5)
    ax.text(9.8, y_top + 0.15, 'v̂(s) 输出', fontsize=9.5, fontweight='bold', color='#1E293B', ha='right')
    
    # ---------------------------------------------------------
    # 3. Control Feedback Loop (y = 0.8 is loop rail)
    # ---------------------------------------------------------
    y_loop = 0.8
    
    # Node to Summing Junction
    plt.plot([9.5, 9.5], [y_top, y_loop], color='#94A3B8', lw=1.5)
    plt.plot([9.5, 8.0], [y_loop, y_loop], color='#94A3B8', lw=1.5)
    ...
    plt.plot([sum_x, sum_x], [0.2, y_loop - 0.22], color='#94A3B8', lw=1.5)
    ax.text(sum_x, 0.05, 'v̂_ref(s) (=0)', fontsize=8.5, color='#475569', ha='center')
    ax.text(sum_x - 0.18, y_loop - 0.18, '+', fontsize=11, color='#64748B', ha='center', va='center')
    
    # (b) Feedback Controller H(s) Block
    ctrl_x = 3.8
    ctrl_w = 2.8
    ...
    # Error signal line
    ax.annotate('', xy=(ctrl_x, y_loop), xytext=(sum_x - 0.22, y_loop),
                arrowprops=dict(arrowstyle="->", color='#94A3B8', lw=1.5))
    ax.text((sum_x - 0.22 + ctrl_x)/2, y_loop + 0.15, 'v̂_e(s)', fontsize=8.5, color='#475569', ha='center')
    ...
    ax.annotate('', xy=(1.9, y_top - 0.45), xytext=(2.0, y_top - 0.6),
                arrowprops=dict(arrowstyle="->", color='#94A3B8', lw=1.5))
    # Line branching to dependent voltage source
    ...
    ax.text(5.5, y_bot - 0.2, '环路增益 T(s)', fontsize=9, color='#64748B', style='italic', ha='center')
    
    plt.savefig("/Users/walter/Downloads/WPSSync/Work/3.Engineering/Ctrl/2PhBuck/DesignDoc/coupled_buck_schematic.png",
                dpi=300, bbox_inches='tight')
    plt.close()
    print("Full circuit schematic successfully drawn and saved.")
